...
export function FloatingCTA({
  ...
  if (instagram) {
    socialLinks.push({
      ...
      href: instagram,
      color:
        "bg-gradient-to-br from-[#833AB4] via-[#FD1D1D] to-[#F77737] hover:opacity-90",
      icon: (
        <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
      ...
      >
        {socialLinks.map((link) => (
          <Link
            ...
            className={`flex h-12 w-12 items-center justify-center rounded-full text-white shadow-lg transition-all duration-300 hover:scale-110 ${link.color}`}
      ...
      >
        <svg
          className={`h-5 w-5 text-white transition-all duration-300 ${
            isExpanded ? "rotate-180 scale-90" : "rotate-0 scale-100"
          }`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
